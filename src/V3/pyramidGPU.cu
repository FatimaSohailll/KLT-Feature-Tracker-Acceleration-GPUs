  ...
  int nlevels)
{
  ...
  for (i = 0 ; i < nlevels ; i++)  {
    pyramid->img[i] =  _KLTCreateFloatImageGPU(ncols, nrows);
    pyramid->ncols[i] = ncols;  pyramid->nrows[i] = nrows;
    ncols /= subsampling;  nrows /= subsampling;
  }
  ...
  _KLT_Pyramid pyramid)
{
  ...
  /* Free GPU images */
  for (i = 0 ; i < pyramid->nLevels ; i++)
    _KLTFreeFloatImageGPU(pyramid->img[i]);
  ...
  float sigma_fact)
{
  ...
  cudaMemcpy(pyramid->img[0]->data, img->data, 
             ncols * nrows * sizeof(float), cudaMemcpyHostToDevice);

  d_currimg = pyramid->img[0];
  for (i = 1 ; i < pyramid->nLevels ; i++)  {
    /* Create temporary image in GPU memory */
    d_tmpimg = _KLTCreateFloatImageGPU(ncols, nrows);
    
    /* Compute smoothed image directly on GPU */
    _KLTComputeSmoothedImageGPU(d_currimg, sigma, d_tmpimg);
    ...
    /* Launch GPU kernel for subsampling */
    _KLTSubsampleImageGPU(d_tmpimg, pyramid->img[i], 
    ...
    _KLTFreeFloatImageGPU(d_tmpimg);
  }
}
