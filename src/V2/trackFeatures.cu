...
__device__ void gpu_computeGradientSum(const float* gradx1, const float* grady1, const float* gradx2, const float* grady2,
  float* out_gradx, float* out_grady, int cols, int rows, float x1, float y1, float x2, float y2, int width, int height){
    int w = width/2;
    int h = height/2;

    int i = threadIdx.x + blockIdx.x * blockDim.x -w;
    int j = threadIdx.y + blockIdx.y * blockDim.y -h;

    if(i < -w || i>w || j< -h || j> h) // check if in range
      return;
...
__device__ void gpu_computeIntensityDifference(const float* img1, const float* img2, int row, int cols, 
...
__device__ void gpu_computeIntensityDifferenceLightingInsensitive(
  const float* img1, const float* img2, float x1, float y1, float x2, float y2,
  int width, int height, int cols, int rows, float* imgDiff){
  int w = width/2;
  int h = height/2;
